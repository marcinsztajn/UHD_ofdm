...
int mycallback(unsigned char * _header, int _header_valid,
...
int UHD_SAFE_MAIN(int argc, char *argv[]){
	...
	ofdmflexframesync fs = ofdmflexframesync_create(M, cp_len, taper_len, NULL,mycallback, NULL);
	...
	desc.add_options()("help", "help message")("args",
			...
			"multi uhd device address args")("nsamps",
			po::value<size_t>(&total_num_samps)->default_value(1000),
			"total number of samples to receive")("rate",
			po::value<double>(&rate)->default_value(2.0e6),
			"rate of incoming samples")("freq",
			po::value<double>(&freq)->default_value(869e6),
			"rf center frequency in Hz")("gain",
			po::value<double>(&gain)->default_value(15),
			"gain for the RF chain")("ant",
			po::value<std::string>(&ant)->default_value("RX"),
			"antenna selection")("subdev", po::value<std::string>(&subdev),
			"subdevice specification")("bw",
			po::value<double>(&bw)->default_value(4e6),
			"analog frontend filter bandwidth in Hz")("port",
			po::value<std::string>(&port)->default_value("7124"),
			"server udp port")("addr",
			po::value<std::string>(&addr)->default_value("192.168.1.10"),
			"resolvable server address")("ref",
			po::value<std::string>(&ref)->default_value("internal"),
			"reference source (internal, external, mimo)")("int-n",
	...
	if (vm.count("bw")) {
		std::cout
				<< boost::format("Setting RX Bandwidth: %f MHz...") % (bw / 1e6)
				<< std::endl;
...
//	std::vector<std::complex<float> > buff(rx_stream->get_max_num_samps());
	std::vector<std::complex<float> > buff(symbol_len);
...
//    while(num_acc_samps < total_num_samps){
	while (1) {
		...
		ofdmflexframesync_execute(fs, (liquid_float_complex*)&buff, symbol_len);
