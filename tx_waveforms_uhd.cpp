...
int UHD_SAFE_MAIN(int argc, char *argv[]){
	...
	spb = 0;
	total_num_samps = 0;
	rate = 2.0e6; 		//sampling rate
	freq = 1.38e9;		//carrier frequency
	ampl = 0.3f;		//optional sine ampl
	gain = 15;			//gain for the RF chain
	ant = "RX/TX"; 		//antenna selection
	bw = rate * 1.2; 		//bandwidth filter in Hz
	wave_type = "SINE";		//arbitrary wave type
	wave_freq = 0.5e6;		//sine frequency
	ref = "gpsdo";		//reference clock
	pps = "gpsdo";		//PPS source
	otw = "sc16";
	channel_list = "0";
...
//    ;
		...
		if (vm.count("gain")) {
			...
			std::cout
					<< boost::format("Actual TX Gain: %f dB...")
							% usrp->get_tx_gain(channel_nums[ch]) << std::endl
					<< std::endl;
		}
	...
	const size_t step = boost::math::iround(
			wave_freq / usrp->get_tx_rate() * wave_table_len);
	size_t index = 0;
	...
	std::cout << boost::format("Samples per buffer : ") % spb << std::endl; //samples per buffer
	//std::cout<<"Samples per buffer : " << spb << std::endl; //alternative version

	/////////////////////////////////////////////////BUFFERS IMPORTANT !!! /////////////////////////////////////////
	std::vector<std::complex<float> > buff(22*80);
	...
	const size_t tx_sensor_chan =
			channel_list.empty() ?
					0 : boost::lexical_cast<size_t>(channel_list[0]);
	...
	if ((ref == "mimo")
			and (std::find(sensor_names.begin(), sensor_names.end(),
					"mimo_locked") != sensor_names.end())) {
		uhd::sensor_value_t mimo_locked = usrp->get_mboard_sensor("mimo_locked",
				mboard_sensor_idx);
		std::cout
				<< boost::format("Checking TX: %s ...")
...
//	std::complex<float> buffer[symbol_len];       // time-domain buffer
//	std::complex<float> buffer_all[22*80];
	...
	ofdmflexframegen fg = ofdmflexframegen_create(M, cp_len, taper_len, NULL,
	NULL);
	...
	while (!last_symbol) {
		...
		last_symbol = ofdmflexframegen_write(fg, (liquid_float_complex*) &buff[idx_ofdm_sym], symbol_len);
		idx_ofdm_sym += 80;
		//wpisuje symbole do buffer_all
//		 print status
...
//        }
		...
		num_acc_samps += tx_stream->send(buffs, buff.size(), md); //send samples(read_only memory_with_samples,
//        size_of_buffer, metadata)
